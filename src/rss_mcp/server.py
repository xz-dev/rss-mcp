...
class RSSMCPServer:
            ...
            return [
                ...
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "name": {"type": "string", "description": "Unique name for the feed"},
                            "urls": {
                                "type": "array",
                                "items": {"type": "string"},
                                "description": "List of RSS source URLs",
                            },
                            "title": {"type": "string", "description": "Optional feed title"},
                            "description": {
                                "type": "string",
                ...
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "name": {"type": "string", "description": "Name of the feed to remove"}
                        },
                        "required": ["name"],
                    },
                ),
                Tool(
                    ...
                    inputSchema={
                        "type": "object",
                        "properties": {
                            "feed_name": {"type": "string", "description": "Name of the feed"},
                            "url": {"type": "string", "description": "RSS source URL"},
                            "priority": {
                                "type": "integer",
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            "feed_name": {"type": "string", "description": "Name of the feed"},
                            "url": {"type": "string", "description": "RSS source URL to remove"},
                        },
                        "required": ["feed_name", "url"],
                    },
                ),
                ...
                Tool(
                    ...
                    inputSchema={
                        ...
                        "properties": {
                            "feed_name": {"type": "string", "description": "Name of the feed"},
                            "entry_guid": {"type": "string", "description": "GUID/ID of the entry"},
                            "max_length": {
                                "type": "integer",
    ...
    async def _handle_add_feed(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        return [
            TextContent(type="text", text=f"✓ Added feed '{name}' with {sources_created} source(s)")
        ]
    ...
    async def _handle_add_source(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        if self.storage.create_source(source):
            ...
            return [
                TextContent(type="text", text="Error: Source already exists or failed to create")
            ]
    ...
    async def _handle_remove_source(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        if self.storage.delete_source(feed_name, url):
            return [
                TextContent(type="text", text=f"✓ Removed source {url} from feed '{feed_name}'")
            ]
        else:
    ...
    async def _handle_get_entries(self, args: Dict[str, Any]) -> Sequence[TextContent]:
        ...
        total_count = self.storage.get_entry_count(
            feed_name=feed_name, start_time=start_time, end_time=end_time
        )
    ...
    async def root():
        return {
            ...
            "endpoints": {
                ...
                "sse": {"feed-updates": "/sse/feed-updates", "tool-calls": "/sse/tool-calls"},
            },
        }
    ...
    async def mcp_endpoint(request: Request):
        ...
        try:
            ...
            if body.get("method") == "initialize":
                return {
                    "result": {
                        "serverInfo": {"name": "rss-mcp-server", "version": "1.0.0"},
                        "capabilities": {"tools": {}},
                    }
                }
                ...
                # Handle tool calls - redirect to call-tool endpoint logic
                ...
                return {"error": {"code": -1, "message": f"Unknown method: {body.get('method')}"}}
