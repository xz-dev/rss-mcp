...
from .conftest import assert_successful_command
...
class TestCLIBasicOperations:
    ...
    def test_feed_management_workflow(self, cli_wrapper):
        ...
        result = cli_wrapper.add_source(
            "test-feed", "https://rsshub.app/github/trending/weekly", priority=1
        )
...
class TestCLIDataConsistency:
    ...
    def test_feed_lifecycle(self, cli_wrapper):
        ...
        # 1. Create feed
        result = cli_wrapper.add_feed(
            feed_name, "https://rsshub.app/36kr/newsflashes", title="Lifecycle Test"
        )
        assert_successful_command(result)
        ...
        sources = ["https://rsshub.app/36kr/latest", "https://rsshub.app/zhihu/hot"]
...
class TestCLIMockNetworkOperations:
    """Test CLI operations without external network (using static mock data)."""

    def test_feed_management_workflow(self, cli_wrapper):
        """Test complete feed workflow without network."""
        # Use a mock URL that won't be actually fetched
        mock_url = "http://localhost:9999/test-feed.xml"
        
        # Add a test feed
        result = cli_wrapper.add_feed(
            "test-feed", mock_url, title="Test Feed"
        )
        assert_successful_command(result)
        assert "Added feed 'test-feed'" in result.stdout

        # List feeds to verify creation
        result = cli_wrapper.list_feeds()
        assert_successful_command(result)
        assert "test-feed" in result.stdout

        # Add another source  
        result = cli_wrapper.add_source("test-feed", "http://localhost:9999/backup.xml", priority=1)
        assert_successful_command(result)
        
        # Get stats (should work even without entries)
        result = cli_wrapper.get_stats("test-feed")
        assert_successful_command(result)
        assert "Statistics for feed 'test-feed'" in result.stdout

        # Note: We skip fetch test since we can't control external network
        # The feed management itself is what we're testing here

    def test_feed_error_handling(self, cli_wrapper):
        """Test CLI error handling without network."""
        # Try to add source to non-existent feed (better test, no interactive prompt)
        result = cli_wrapper.add_source("does-not-exist", "http://example.com/rss.xml")
        assert result.returncode != 0
        assert "not found" in result.stderr.lower()
        
        # Try to get stats for non-existent feed  
        result = cli_wrapper.get_stats("does-not-exist")
        assert result.returncode != 0
        assert "not found" in result.stderr.lower()
...
class TestCLINetworkOperations:
    ...
    def test_feed_fetch_success(self, cli_wrapper):
        ...
        result = cli_wrapper.add_feed(
            "github-trending", "https://rsshub.app/github/trending/daily", title="GitHub Trending"
        )
        assert_successful_command(result)
    ...
    def test_feed_failover(self, cli_wrapper):
        ...
        result = cli_wrapper.add_feed(
            "failover-test", "https://httpbin.org/status/500", title="Failover Test"  # Always fails
        )
        assert_successful_command(result)

        # Add good backup source
        result = cli_wrapper.add_source(
            "failover-test", "https://rsshub.app/github/trending/daily", priority=1  # Should work
        )
        assert_successful_command(result)
