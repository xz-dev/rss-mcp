...
from .mock_rss_server import MockRSSServer, get_mock_feed_urls
...
@pytest.fixture
async def mock_rss_server():
    """Create a mock RSS server for testing."""
...
@pytest.fixture  
def mock_test_feeds(mock_rss_server):
    """Mock RSS feed URLs using local server."""
    return get_mock_feed_urls(mock_rss_server.get_url())


@pytest.fixture
def test_feeds():
    """Test RSS feed URLs - updated to use more reliable sources."""
    return {
        "bbc-world-asia": {
            "name": "bbc-world-asia", 
            "title": "BBC World Asia News",
            "urls": ["https://rss.owo.nz/bbc/world-asia"],
            "backup_urls": ["https://rss.owo.nz/bbc/world"],
        },
        "github-trending": {
        ...
        "v2ex-latest": {
            ...
            "urls": ["https://rsshub.app/v2ex/topics/latest"], 
            "backup_urls": [],
        },
    }
...
class ServerProcess:
    ...
    def __init__(
        self, config_path: Path, mode: str = "stdio", host: str = "127.0.0.1", port: int = 8080
    ):
...
def cli_wrapper(temp_dir, integration_config):
    ...
    class CLIWrapper:
        ...
        def add_source(
            self, feed_name: str, url: str, priority: int = 0
...
def assert_successful_command(result: subprocess.CompletedProcess, expected_text: str = None):
    ...
    if expected_text:
        ...
        ), f"Expected '{expected_text}' not found in output: {result.stdout}"
